...
def extract_icrd(filepath):
    with open(filepath, "rb") as f:
        ...
        if idx != -1:
            ...
            try:
                return datetime.strptime(icrd_str, "%Y-%m-%dT%H:%M:%SZ").replace(
                    tzinfo=timezone.utc
                )
            except ValueError:
                pass
...
def main():
    ...
    if args.json:
        ...
        for mac, stats in results.items():
            print(
                f"{mac:<18} | {stats['time_coverage_pct']:<12.1f} | {stats['railing_pct']:<12.1f} | {stats['total_files']:<6} | {len(stats['gaps'])}"
            )
            for g in stats["gaps"]:
